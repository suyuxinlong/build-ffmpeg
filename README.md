...
*   **VVC Module Masking**: The VVC (H.266) decoder introduced in FFmpeg 7.1 contains a lot of new AArch64 assembly that currently has compatibility issues with the iOS cross-compilation toolchain. The script automatically avoids this compilation error via `--disable-vvc` to ensure overall build success.
...
*   **Bitcode**:
    The script enables `-fembed-bitcode` by default. Although Xcode 14+ has deprecated Bitcode, keeping this option is safe for compatibility with old projects. Bitcode increases the size of `.a` files but **does not** increase the size of the final App Store distribution package (Apple handles stripping on the server side).
...
```text
/Users/suyuxinlong/Developer/Git/build-ffmpeg/
